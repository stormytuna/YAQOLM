...
public class ShopGlobalNPC : GlobalNPC
{
    public override void ModifyShop(NPCShop shop) {
        if (shop.NpcType == NPCID.Steampunker && ServerConfig.Instance.SteampunkerSolutions) {
            ...
            shop.InsertAfter(ItemID.GreenSolution, ItemID.DarkBlueSolution);
        }
        ...
        if (shop.NpcType == NPCID.DD2Bartender && ServerConfig.Instance.CheaperDefendersForge) {
            NPCShop.Entry defendersForgeEntry = shop.Entries.FirstOrDefault(entry => entry.Item.type == ItemID.DefendersForge);
            if (defendersForgeEntry is not null) {
                defendersForgeEntry.Item.shopCustomPrice = 15;
            }
        }
    }
