...
public class YAQOLMGlobalItem : GlobalItem
{
	...
	public override bool CanRightClick(Item item) {
		if (ArraySystem.ItemsThatPlaceTilesWithRecipes.Contains(item.type) && !Main.LocalPlayer.GetModPlayer<ConsumableCraftingStationsPlayer>().HasConsumedItem(item) &&
		    ServerConfig.Instance.InventoryCraftingStations) {
			return true;
		}
	...
	public override void ModifyTooltips(Item item, List<TooltipLine> tooltips) {
		if (item.type == ItemID.Goggles && ServerConfig.Instance.GogglesGiveNightVision) {
			TooltipLine tip = tooltips.FindLast(t => t.Mod == "Terraria");
			tip.Text += "\nGrants Night Owl";
			return;
		}

		if (item.type == ItemID.SharpeningStation && ServerConfig.Instance.BuffStationChanges) {
			TooltipLine tip = tooltips.FirstOrDefault(t => t.Mod == "Terraria" && t.Name == "Tooltip0");
		...
		if (item.type == ItemID.CrystalBall && ServerConfig.Instance.BuffStationChanges) {
			TooltipLine tip = tooltips.LastOrDefault(t => t.Mod == "Terraria");
		...
		if (item.type == ItemID.LifeFruit && ServerConfig.Instance.BetterLifeFruit) {
			TooltipLine tip = tooltips.FirstOrDefault(t => t.Name == "Tooltip0");
