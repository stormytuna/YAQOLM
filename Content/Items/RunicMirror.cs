...
public class RunicMirror : ModItem
{
    ...
    public override void UpdateInventory(Player player) => player.GetModPlayer<RunicMirrorPlayer>().runicMirror = true;
}
...
public class RunicMirrorPlayer : ModPlayer
{
    ...
    public override void ResetEffects() => runicMirror = false;
...
public class RunicMirrorGlobalNPC : GlobalNPC
{
    ...
    public override void ModifyNPCLoot(NPC npc, NPCLoot npcLoot) => npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<RunicMirror>()));
}
...
public class RunicMirrorSystem : ModSystem
{
    ...
    private bool On_PlayerOnInInteractionRange(On_Player.orig_InInteractionRange orig, Player self, int interactX, int interactY, TileReachCheckSettings settings) {
        ...
        return (TileID.Sets.CountsAsPylon.Contains(tile.TileType) && self.HasItem(ModContent.ItemType<RunicMirror>())) || (self.HasItem(ModContent.ItemType<SpiralMirror>()) && !ret)
|| ret;
    }
    ...
    private bool On_TeleportPylonsSystemOnIsPlayerNearAPylon(On_TeleportPylonsSystem.orig_IsPlayerNearAPylon orig, Player player) {
        return player.HasItem(ModContent.ItemType<RunicMirror>()) || player.HasItem(ModContent.ItemType<SpiralMirror>())
|| orig(player);
    }
}